    ...
    <section className="lf-surface relative overflow-hidden rounded-3xl border p-7">
      ...
      <div className="relative z-10 max-w-2xl">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-(--color-accent)/20 bg-(--color-accent)/10 px-3 py-1 text-xs text-(--color-accent)">
          ...
          {t("dashboard.hero_panel.premium_toolkit", "Premium Game Toolkit")}
        </div>

        <h1 className="text-4xl font-bold tracking-tight text-(--color-text)">
          {t("dashboard.hero_panel.heading", "Gestiona tus juegos, paquetes Lua y manifests desde un solo lugar.")}
        </h1>

        <p className="mt-3 max-w-xl text-sm leading-6 text-(--color-muted)">
          {t("dashboard.hero_panel.description", "Detecta Steam, instala paquetes desde API, administra descargas, revisa actividad y ejecuta tus juegos desde una interfaz moderna.")}
        </p>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <button className="inline-flex items-center gap-2 rounded-xl bg-(--color-accent) px-5 py-3 text-sm font-medium text-(--color-accent-text) transition hover:opacity-90">
            ...
            {t("dashboard.hero_panel.explore_packages", "Explorar paquetes")}
          </button>
          ...
          <button className="inline-flex items-center gap-2 rounded-xl border border-(--surface-active-border) bg-white/6 px-5 py-3 text-sm text-(--color-text) backdrop-blur-xl transition hover:bg-white/10">
            ...
            {t("dashboard.hero_panel.view_library", "Ver biblioteca")}
          </button>
        </div>
      </div>
    </section>
