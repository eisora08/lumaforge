...
export default function CardActionMenu({
  ...
  return createPortal(
    <div
      ref={menuRef}
      role="menu"
      className={`fixed z-[55] w-[184px] overflow-hidden rounded-xl border border-(--surface-active-border)/60 bg-(--color-surface)/95 p-1 shadow-2xl backdrop-blur-xl ${closing ? "lf-popover-exit" : "lf-popover-enter"}`}
...
function SubmenuPanel({
  ...
  return createPortal(
    <div
      data-submenu
      className="fixed z-[60] w-[172px] overflow-hidden rounded-xl border border-(--surface-active-border)/60 bg-(--color-surface)/95 p-1 shadow-2xl backdrop-blur-xl lf-popover-enter"
      style={{ top: pos.top, left: pos.left }}
    >
      {items.map((item, i) => (
