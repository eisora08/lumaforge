...
pub fn slssteam_full_setup(app_handle: tauri::AppHandle) -> Result<String, String> {
    ...
    // 2. Normalize first so get_additional_apps can parse existing entries
    let _ = slssteam_config::normalize_config();

    // 3. Preserve existing AdditionalApps entries before rewriting
    let config_path = slssteam_config::get_config_path();
    let existing_apps: Vec<String> = if config_path.exists() {
        slssteam_config::get_additional_apps().unwrap_or_default()
    } else {
        vec![]
    };

    // 4. Always write a clean default config (fixes any corruption)
...
"#;
    ...
    // 5. Re-add any preserved AdditionalApps entries
    for app_id in &existing_apps {
        let _ = slssteam_config::add_additional_app(app_id, "");
    }
